...
def main(arg):
  ...
  for url_product in urls:
    ...
    for section in accordion_sections:
      ...
      if arg == 'false':
        ...
        if header.find('Especificações') != -1:
          content['especificacoes'].append(text)
        elif header.find('Acessórios') != -1:
          content['acessorios'].append(text)
        elif header.find('Itens Incluidos') != -1:
          content['items_incluidos'].append(text)
        elif header.find('Compatibilidades') != -1:
          content['compatibilidades'].append(text)
      else:
        text = accordion_inner_text
        header = accordion_heading_text  
        if header.find('">Especificações<') != -1:
          print(header)
          content['especificacoes'].append(text.get_text(strip=True))
        elif header.find('>Acessórios<') != -1:
          content['acessorios'].append(text)
        elif header.find('>Itens Incluidos<') != -1:
          content['items_incluidos'].append(text)
  ...
  if arg == 'false':
    excel_file = 'equipamentos_laboratorio.xlsx'
  else:
    excel_file = 'equipamentos_laboratorio_formatted.xlsx'
